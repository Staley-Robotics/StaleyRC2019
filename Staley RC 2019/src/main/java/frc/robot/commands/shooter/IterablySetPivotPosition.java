 ...
 */
public class IterablySetPivotPosition extends Command {
  ...
  private PivotTargets[] pivotTargets = { PivotTargets.GROUND, PivotTargets.LOW,
      PivotTargets.MID, PivotTargets.HIGH, PivotTargets.CARGO };

  public IterablySetPivotPosition(int iter) {
    oi = OI.getInstance();
  ...
  protected void initialize() {
    ...
    if (inBounds(index + iter)) {
      ...
      if (index < 0) {
        ...
        index = pivotTargets.length - 1;
      }
    }
    
    if (pivotTargets[index].getAngle() < shooter.getPivotAngle() - tolerance && iter >= 0) {
      while (pivotTargets[index].getAngle() < shooter.getPivotAngle() - tolerance) {
        index += iter;
        if (index > pivotTargets.length - 1) {
          index = 0;
          break;
        }
      }
    } else if (pivotTargets[index].getAngle() > shooter.getPivotAngle() + tolerance && iter < 0) {
      while (pivotTargets[index].getAngle() > shooter.getPivotAngle() + tolerance) {
        index += iter;
        if (index < 0) {
          index = pivotTargets.length - 1;
