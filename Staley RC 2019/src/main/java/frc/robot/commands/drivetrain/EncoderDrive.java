...
public class EncoderDrive extends Command {
    ...
    protected void execute() {
        ...
        //double leftSign = Math.signum(desiredLeftEncoderPulses-driveTrain.getLeftPosition());
        //double . = Math.signum(desiredRightEncoderPulses - driveTrain.getRightPosition());
		...
		if (leftError < pThreshold || rightError <pThreshold) {
            driveTrain.tankDrive(leftError*lKP,rightError*rKP);
             ...
             //solid chance I messed one of these up
                driveTrain.tankDrive(leftError<-1?-1+SAFETYSPEEDREDUCER:leftError>1?1-SAFETYSPEEDREDUCER:leftError
    ...
    protected boolean isFinished() {
        ...
        if (Math.abs(leftError - desiredLeftEncoderPulses) <= stopThreshold && Math.abs(rightError - desiredRightEncoderPulses) <= stopThreshold) 
			System.out.println("Success Obtained: " + leftError + " , " + desiredLeftEncoderPulses);
		return Math.abs(leftError - desiredLeftEncoderPulses) <= stopThreshold && Math.abs(rightError - desiredRightEncoderPulses) <= stopThreshold;
    }
